...
import { mdLabel, mdNodeId, mdPath, syncMdFoot } from '../node/md-overlay.js';
import { showToast } from '../shell/toasts.js';


function persistPack() {
    store.setRaw('pack', JSON.stringify([...S.pack].map(([id, label]) => ({ id, label }))));
}
...
export function setupPack() {
    ...
    if (scopeBtn) {
        ...
        scopeBtn.onclick = () => {
            ...
            store.setRaw('graphScope', S.graphScope);
            sync();
