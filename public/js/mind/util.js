...
// Znak značky — geometria žije v shared/ (dôvod pri re-exporte nižšie).
// Import, nie len `export … from`: `loadingHtml()` v tomto súbore `sigilNetMarkup`
// SÁM VOLÁ, a `export { x } from` lokálnu väzbu NEVYTVORÍ — bola by to
// ReferenceError pri prvom načítaní zoznamu.
import { sigilNetMarkup, sigilNetSvg } from '../shared/sigil.js';
...
/* ZNAK HADESA — SIEŤ PAMÄTI. Geometria (`SIGIL_NET`), `sigilNetMarkup()`
   a `sigilNetSvg()` sa 2. 9. 2026 PRESUNULI do `public/js/shared/sigil.js`
   a je to funkčný presun, nie upratovanie: na `mind/*` dosiahnu len moduly
   obrazovky Graf, takže `/console` a `/chat` na výkres import nemali a ich
   nosiče kreslili starý prstenec s mŕtvou triedou `bc-ring`. Celý komentár
   s nameranými dôvodmi (pravidlo redukcie po stupňoch, kontrakt s kresbou
   `bc-*`, prečo `pathLength="100"` na každej hrane) je tam.

   Re-export je zámerný a nie je to shim: volajúci na obrazovke Graf
   (`mind/charon.js`, `loadingHtml()` nižšie) môžu importovať odtiaľ aj odtiaľ,
   ale zdroj kresby je JEDEN. */
export { sigilNetMarkup, sigilNetSvg };
